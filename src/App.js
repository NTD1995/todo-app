...
const handleAddTodo = () => {
setTodos([...todos, { id: todoId, title: todoTitle, status: done, detail: detail }])
setTodoId(todoId + 1)
setTodoTitle('')
}
...
function App() {
  const [todos, setTodos] = useState([{id: 1, title: "Todo1", status: done, detail: detail }
]);
...
const handleAddTodo = () => {
 ...
 if(name === "") return
 setTodos((prevTodos) => {
   return [...prevTodos, { id: uuidv4(), title: title, status: done, detail: detail }];
 });
